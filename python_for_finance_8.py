...
def visualize_data():
    ...
    #create a correlation table of the dataframe
    df_corr = df.corr()
    #print(df_corr.head())

    data = df_corr.values
    fig = plt.figure()
    #plot 1 by 1 and plot number 1. other implementation is ....(1,1,1)
    ax = fig.add_subplot(111)
    heatmap = ax.pcolor(data, cmap=plt.cm.RdYlGn)
    fig.colorbar(heatmap)
    ax.set_xticks(np.arange(data.shape[0]) + 0.5, minor=False)
    ax.set_yticks(np.arange(data.shape[1]) + 0.5, minor=False)
    #remove the gap of the top of the matplotlib graph
    ax.invert_yaxis()
